...
object Parser {
  ...
  val expressionParser: Parser[EllsType] = P(
    nilParser | numberParser | stringParser | booleanParser | identityParser | listParser | quoteParser | unquoteParser)

  val numberParser: Parser[EllsType] = P(doubleParser | longsParser)
  val identityParser: Parser[EllsType] = P(
    ((charParser | specialCharsParser) ~ (digitsParser | specialCharsParser | charParser).rep).!.map(id =>
      EllsIdentifier(id)))
  val stringParser: Parser[EllsType] = P(
    (separatorParser ~ "\"" ~/ (strCharsParser | escapeParser).rep.! ~ "\"").map(str =>
      EllsString(StringContext.treatEscapes(str))))
  ...
  val listParser: Parser[EllsType] =
    P("(" ~/ expressionParser.rep(sep = separatorParser) ~ separatorParser ~ ")").map(v => EllsList(v.toList))
  ...
  private val quoteParser: Parser[EllsType] =
    P("'" ~ expressionParser).map(v => EllsList(List(EllsIdentifier("quote"), v)))
  private val unquoteParser: Parser[EllsType] =
    P("@" ~ expressionParser).map(v => EllsList(List(EllsIdentifier("unquote"), v)))
  ...
  private val doubleParser: P[EllsDouble] = P(
    ("-".? ~ (digitsParser ~ fractionalParser ~ exponentParser.?)).!.map(d => EllsDouble(d.toDouble)))
