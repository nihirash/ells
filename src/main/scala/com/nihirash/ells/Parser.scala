...
object Parser {
  ...
  val stringParser: Parser[EllsType] = P((sepparatorParser ~ "\"" ~/ (strCharsParser | escapeParser).rep.! ~ "\"").map(str => EllsString(StringContext.treatEscapes(str))))
  ...
  val listParser: Parser[EllsType] = P(("(" ~/ valueParser.rep(sep = sepparatorParser) ~ sepparatorParser ~ ")")).map(v => EllsList(v.toList))
